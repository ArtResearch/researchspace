      ...
      D.div({ className: 'facet__relation__values__filter', style: { height: '500px' } },
        createElement(LazyTreeWithSearch, {
          ...
          onSelectionChanged: (selection: TreeSelection<Node>) => {
            const next = TreeSelection.leafs(selection)
              .map(n => ({ iri: n.iri, tuple: n.tuple, label: n.label?.value }))
              .toArray();
            this.props.actions.setRelationSelections(this.props.relation)(next);
          }          
        }
        ),
      )
