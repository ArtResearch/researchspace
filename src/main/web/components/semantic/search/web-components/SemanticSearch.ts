  ...
  private setBaseQuery = (query: Data.Maybe<SparqlJs.SelectQuery>) => {
    this.setState(
      (state): State => {
        ...
        // when we have facets then base query is not a result query
        // result query will be generated by the facets
        const baseState = state.facetActions ? {} : { resultQuery: query, resultsLoaded: false};
        return {
          ...baseState,
          baseQuery: query,
          ...
          isConfigurationEditable: true, //query.isJust ? false : true,
        };
      }
    );
  };
