...
export class LazyTreeWithSearch extends Component<LazyTreeWithSearchProps, State> {
  ...
  componentWillReceiveProps(newProps: LazyTreeWithSearchProps) {
    if (newProps.rootsQuery !== this.props.rootsQuery) {
      ...
      const currentLeafs = TreeSelection.leafs(this.state.confirmedSelection).map(n => n.iri.value).toSet();
      const newLeafs = Immutable.Set(newProps.initialSelection.map(s => typeof s === 'string' ? s : s.value));

      if (!Immutable.is(currentLeafs, newLeafs)) {
  ...
  private requestChildren(path: KeyPath) {
    ...
    const displayingSearch = !!this.state.searchResult;
    this.updateForest(
      displayingSearch,
      (forest) => {
      ...
      () => {
        this.cancellation.map(changePromise).onValue((change) =>
          this.updateForest(displayingSearch, (forest) => change(forest))
        );
      }
    );
  }
    ...
    return LabelsService.getLabels(leafs.map(node => node.iri), { context: this.context.semanticContext })
      .flatMap(labels => {
        ...
        return this.state.model
          ...
          .map((treeRoot) => {
            // Convert leaves of the selection tree to terminal nodes (children = undefined)
            // to ensure implicit selection propagation works correctly.
            const makeTerminal = (node: Node): Node => {
              if (node.children && node.children.length > 0) {
                return { ...node, children: node.children.map(makeTerminal) };
              }
              return { ...node, children: undefined };
            };
            return KeyedForest.create(Node.keyOf, sealLazyExpanding(makeTerminal(treeRoot)));
          });
      })
      .toProperty();
  }
