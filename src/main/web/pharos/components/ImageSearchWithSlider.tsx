...
import { FormControl, Button, Dropdown, MenuItem, Checkbox, DropdownButton, Popover, OverlayTrigger } from 'react-bootstrap';
...
interface State {
  ...
  showImageSearchModal: boolean; // Whether to show the image search modal overlay
  imageSearchUrl: string; // URL input in the modal
}
...
class ImageSearchWithSliderInner extends React.Component<InnerProps, State> {
  ...
  private updateOverlayPosition = () => {
    if (!this.overlayWrapperRef || !this.state.showImageSearchModal) {
      return;
    }

    // Use input container if available, otherwise use image preview container
    const referenceElement = this.inputContainerRef || this.imagePreviewContainerRef;
    if (!referenceElement) {
      return;
    }
    ...
    const top = referenceRect.top - containerRect.top; // Start at the same top as input

    // On smaller screens, ensure overlay doesn't overflow container
    const containerWidth = containerRect.width;
    if (left + width > containerWidth) {
      width = Math.max(containerWidth - left, 200); // Minimum width of 200px
    }
    
    // Ensure overlay doesn't go negative on the left
    if (left < 0) {
      left = 0;
      width = Math.min(referenceRect.width, containerWidth);
    }

    // Apply styles to overlay wrapper
    this.overlayWrapperRef.style.left = `${left}px`;
    this.overlayWrapperRef.style.top = `${top}px`;
    ...
    // Reset transform since we're positioning absolutely
    ...
    return (
      <div className={styles.imageSearchWithSlider}>
        ...
        <div className={styles.imageSearchContainer}>
          {/* Always show input, but with opacity when overlay is active */}
          {!this.state.showImage && this.state.progress <= 0 && (
            <>
              {/* Input with search icon and camera icon */}
              <div 
                ref={(ref) => { this.inputContainerRef = ref; }}
                className={`${styles.inputDropzoneContainer} ${this.state.showImageSearchModal ? styles.inputDisabled : ''}`}
              >
                ...
                <div className={styles.inputWithClear}>
                  <FormControl
                        ...
                        type="search"
                        autoFocus={!this.props.initialInput || _.isEmpty(this.props.initialInput)}
                        className={className}
                        style={style}
                        value={this.state.value}
                        placeholder={placeholder}
                        onChange={this.onKeyPress}
                        disabled={this.state.showImageSearchModal}
                  />
                  {this.state.value &&
                    <Icon
              ...
              <Button className={styles.searchButton} onClick={this.handleSearch} disabled={this.state.showImageSearchModal}>
                Search
              </Button>
            </>
          )}
          
          {/* Image Search In-Place Overlay */}
          {this.state.showImageSearchModal && (
            <div 
              ref={(ref) => { this.overlayWrapperRef = ref; }}
              className={styles.imageSearchOverlayWrapper}
            >
              <div className={styles.imageSearchOverlay}>
                ...
                <div className={styles.imageSearchOverlayBody}>
                  ...
                  <Dropzone
                    accept={"image/*"}
                    onDropAccepted={this.handleDropFromModal}
                  ...
                  <div className={styles.imageSearchUrlContainer}>
                    <FormControl
                      ...
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') {
                          this.handleSearchFromModalUrl();
                        }
                      }}
                    />
                    <Button 
                      className={styles.imageSearchModalSearchBtn}
                      onClick={this.handleSearchFromModalUrl}
                    >
                      Search
                    </Button>
                  </div>
          ...
          {this.state.showImage ? (
            ...
            <div 
              ref={(ref) => { this.imagePreviewContainerRef = ref; }}
  ...
  private handleDrop = (files: File[]) => {
    ...
    if (files && files.length > 0) {
      ...
      this.imageUploadService.uploadImage(file, this.props.imageStorage, (progress) => {
        // Cap progress at 99% during upload to keep progress bar visible
        // until server response is received
        this.setState({ progress: Math.min(progress, 99) });
      })
        .then(response => {
          // Hide progress bar after server response is fully processed
          this.setState({
  ...
  private handleClearImage = () => {
    ...
    this.setState({
      ...
      this.keys('');
      // Focus input after clearing image, ensuring DOM has updated
      requestAnimationFrame(() => {
        if (this.textInput) {
  ...
  private handleClearText = () => {
    this.setState({
      ...
      this.keys('');
      // Focus input after clearing, ensuring DOM has updated
      requestAnimationFrame(() => {
        if (this.textInput) {
          this.textInput.focus();
        }
  ...
  private handleSearchFromModalUrl = () => {
    const { imageSearchUrl } = this.state;
    if (imageSearchUrl && imageSearchUrl.trim()) {
      // Set the image URL and close modal
      this.setState({
        imageUrl: imageSearchUrl.trim(),
        imageData: undefined,
        showImage: true,
        isImageMode: true,
        value: imageSearchUrl.trim(),
        showImageSearchModal: false,
        imageSearchUrl: ''
      }, () => {
        this.keys(this.state.value);
  ...
  private handleDropFromModal = (files: File[]) => {
    if (!this.state.withImages) {
      console.warn('Image upload is not enabled for this domain');
      return;
    }
    ...
    if (files && files.length > 0) {
      const file = files[0];
      // Close modal immediately when file is selected/dropped
      // Progress bar will still show independently
      this.setState({ 
        progress: 0,
        showImageSearchModal: false,
        imageSearchUrl: ''
  ...
  private buildKeywordQuery = () => (token: string): SparqlJs.SelectQuery | null => {
    ...
    // NOTE: We don't currently support double-quoted grouping in queries.
    // Strip double quotes before tokenization to avoid incorrect grouping semantics downstream.
    const sanitizedToken = token.replace(/"/g, '');
    const keywords = luceneTokenize(sanitizedToken, escapeLuceneSyntax, tokenizeLuceneQuery, minTokenLength);
    ...
    const bindings: Record<string, any> = {
      ...
      // Use the sanitized token (quotes removed) to prevent incorrect results.
      [searchTermVariable!]: Rdf.literal(sanitizedToken),
    };
